...
const AnimatedButton: React.FC<Props> = ({ children, href, invert }) => {
  return (
    <Link
      href={href}
      className={`px-7 text-xs xs:text-sm sm:text-base sm:px-[35px] rounded-full py-2 sm:py-3  text-white group overflow-clip relative z-[1] flex items-center ${
    ...
    >
      ...
      <span className="relative -translate-x-[5px] group-hover:translate-x-[10px] transition-all duration-500">
        {children}
      </span>
      <span
